...
First: it will work whether or not the list really does have just a single value in each element.

Second: The output can be of any type (character, integer etc.)
</div>
...
<div class="notes">
The output from `map_dbl()` is a numeric vector instead of a list.
...
You can also output a tibble by calling `map_dfr()` or `map_dfc()`.I'll show an example of that a bit later …
</div>
...
<div class="notes">
There is **WAY** more to `purrr`, including functions for working with two lists at once, more than two lists, and loads more. But to keep to a sensible time I'm going to switch to an example of using `purrr` and functional programming for something closer to real work.
</div>
...
We can do this with functional programming and `purrr`!
...
<div class="notes">
This is the use of `map_dfr()` that I mentioned earlier. It binds the two tibbles together row-wise, so we get a single tibble of 42 rows.
</div>
...
<div class="notes">
Then just map that function on to the predictions. See at the end I can pass the extra argument for the actual values as an extra named argument.
</div>
...
Let's be conservative about these results and go with the value of alpha that gives the best results with lambda.1se, and check its performance on the test dataset. Remember that lambda.1se has a stronger penalty, so it should generalise better than the minimum.
